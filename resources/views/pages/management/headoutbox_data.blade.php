...
<div class="row">
	<div class="col-md-12 col-sm-12 col-xs-12">
		<div class="x_panel">
		  ...
		  <div class="x_content">
			{{ Form::model($data, ['id'=>'form', 'route'=>!empty($data['receiver_id']) ? ['headoutbox.update', $data['receiver_id']] : ['headoutbox.create', null], 'action'=>'HeadOutboxController@save', 'class'=>'form-horizontal form-label-left']) }}
			...
			<div class="row">	
				<div class="col-sm-6 col-xs-12">
					...
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Nomor Agenda') }}</div>
						<div class="col-sm-8 col-xs-7">{{ $data['outbox_agenda'] }}</div>
					</div>
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Tanggal Surat') }}</div>
					...
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Sifat Naskah') }}</div>
...
@if (!empty($data['outbox_drafts']))
@foreach ($data['outbox_drafts'] as $row => $draft)
@if (!empty($draft['files']))
						<hr>
						<p>Draft # {{ (sizeof($data['memo_drafts']) - $row) }}. Pengunggah: {{ $draft['author']." - ".$draft['date'] }}</p>
						<ul class="list-unstyled">
...
@if ($data['outbox_method'] == "attachment" && !empty($data['outbox_status']) && ($data['outbox_status'] == "draft" || $data['outbox_status'] == "corrector" || $data['outbox_status'] == "user"))
					<div class="form-group form-line">
...
@endforeach
					...
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Pengoreksi Terakhir') }}</div>
						<div class="col-sm-8 col-xs-7">{{ $data['outbox_corrector_name'] }} <span class="pull-right"><small>{{ $data['outbox_corrector_date_text'] }}</small></span></div>
					</div>
@if (!empty($data['outbox_corrector_note']) && ($data['outbox_status'] == "draft" || $data['outbox_status'] == "corrector"))
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Nota Pengoreksi') }}</div>
						<div class="col-sm-8 col-xs-7">{!! $data['outbox_corrector_note'] !!}</div>
					</div>
@endif
@if (!empty($data['outbox_note']) && $data['outbox_status'] == "corrector")
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Nota Penandatangan') }}</div>
						<div class="col-sm-8 col-xs-7">{!! $data['outbox_note'] !!}</div>
					</div>
@endif
@endif
					<div class="form-group form-line">
...
@if (!empty($data['outbox_verificator_name']))
					<div class="form-group form-line">
...
@if (!empty($data['outbox_verificator_note']) && (($data['outbox_status'] == "user" && $data['user_status'] == "user") || ($data['outbox_status'] == "operator" && $data['user_status'] == "operator")))
					<div class="form-group form-line">
						<div class="col-sm-4 col-xs-5">{{ Form::label('Nota Pemeriksa') }}</div>
						<div class="col-sm-8 col-xs-7">{!! $data['outbox_verificator_note'] !!}</div>
					</div>
@endif
@endif
				</div>
