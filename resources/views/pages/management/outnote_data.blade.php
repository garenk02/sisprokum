...
@if (!empty($data['memo_drafts']))
@foreach ($data['memo_drafts'] as $row => $draft)
...
@foreach ($data['memo_corrector'] as $corrector)
						<li>{{ (sizeof($data['memo_corrector']) > 1 ? "- " : "").$corrector['name']." | ".$corrector['post']." | ".$corrector['unit'] }}</li>
@endforeach
						</ul>
						</div>
					</div>
@if (!empty($data['memo_corrector_note']) && ($data['memo_status'] == "draft" || $data['memo_status'] == "corrector") && ($data['user_status'] == "drafter" || $data['user_status'] == "corrector"))
					<div class="form-group form-line">
...
@endif
@if (!empty($data['memo_note']) && $data['memo_status'] == "corrector" && $data['user_status'] == "corrector")
					<div class="form-group form-line">
						<div class="col-xs-4">{{ Form::label('Komentar Penandatangan') }}</div>
						<div class="col-xs-8">{!! $data['memo_note'] !!}</div>
					</div>
@endif
				</div>
				<div class="col-sm-6 col-xs-12">
...
@if (!empty($data['memo_content']))
			<div class="row">
				<div class="col-xs-12">
					...
					<div role="tabpanel">
@if (!empty(sizeof($data['memo_drafts'])))
					<ul class="nav nav-tabs bar_tabs" role="tablist">
					<li class="active"><a href="#draft_now" role="tab" class="list_tab" data-toggle="tab" aria-expanded="true">Terbaru</a></li>				
@foreach ($data['memo_drafts'] as $row => $draft)
					<li class=""><a href="#draft_{{ $row }}" role="tab" class="list_tab" data-toggle="tab" aria-expanded="false">{{ '#'.(sizeof($data['memo_drafts']) - $row).". ".$draft['author']." - ".$draft['date_tab'] }}</a></li>
@endforeach
					</ul>
@endif
					</div>					
					<div class="form-group">
						<div class="col-xs-12 well table-responsive tab-content">
							<div id="draft_now" role="tabpanel" class="letter tab-pane fade active in">
@include('pages/management/note_header')
@include('pages/management/note_view', $data)
							</div>
@if (!empty($data['memo_drafts']))
@foreach ($data['memo_drafts'] as $row => $draft)
							<div id="draft_{{ $row }}" role="tabpanel" class="letter tab-pane fade">
@include('pages/management/note_header')
@include('pages/management/note_draft', $draft)
							</div>
@endforeach	
@endif
						</div>
