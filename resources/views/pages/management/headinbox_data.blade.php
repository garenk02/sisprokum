...
@if (!empty($data['inbox_secretary_note']))
				<div class="form-group form-line">
					<div class="col-sm-3 col-xs-4">{{ Form::label('Note Sekretaris') }}</div>
					<div class="col-sm-9 col-xs-8">{!! $data['inbox_secretary_note'] !!}</div>
				</div>
@endif
			</div>
...
@if (!empty($profile['user_modules'][$module['module_id']]) && in_array('update', $profile['user_modules'][$module['module_id']]) && in_array($data['calendar_user'], $profile['user_posts']))
